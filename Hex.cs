...
namespace HexGrid
{
    ...
    {        
        ...
        {
            var adjustedVector = this - focal;

            var rotated = adjustedVector.Rotate(dir);
        ...
        public Dictionary<Hex, int> FloodFill(int maxRange, List<Hex> obstacles = null)
        {
            ...
            // init return value with starting location
            var ret = new Dictionary<Hex, int> { { this, 0 } };
            var prev = new List<Hex> { this };

            var _this = this;
            if (obstacles.Any(x => x == _this))
                return ret;
            ...
            for (int i = 1; i <= maxRange; i++)
            {
                ...
                foreach (Hex hex in prev)
                {
                    ...
                    foreach (Hex candidate in adjacentCandidates)
                    {
                        ...
                        if (obstacles.Any(x => x == candidate)) continue;
        ...
        public override bool Equals(object obj) { return (obj is Hex hex) ? this == hex : false; }
